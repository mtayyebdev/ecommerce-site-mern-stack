...
function CheckoutCart({data,deleteHandler}) {
    return (
        ...
        <div className="bg-white">
            ...
            <div className="p-3 rounded-md">
                ...
                <div className="flex gap-3 border border-blue-500 p-2 rounded-md w-[250px]">
                    ...
                    <div>
                        <h3 className="text-base font-semibold">Rs. {data.deliveryPrice}</h3>
                        <p className="text-sm text-gray-500 mt-1">Standard Delivery</p>
                        <p className="text-xs text-gray-500 mt-4">Guaranteed {data.guarantee}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row justify-between px-3 mb-6 mt-5">
                <div className="flex">
                    ...
                    <div className="flex flex-col ml-4">
                        ...
                        <p className="text-xs text-gray-500">No Brand, Color Family: {data.color}</p>
                    </div>
                </div>
                ...
                <div className="flex flex-row mt-4 w-full sm:w-[50%] md:w-full justify-between sm:mt-0 md:mt-4 sm:ms-3 md:ms-0 lg:ms-3 lg:mt-0 lg:w-[50%] xl:w-[30%]">
                    <div className='flex flex-col'>
                        <p className="font-semibold text-lg text-site-color whitespace-nowrap">Rs. {data.price}</p>
                        <p className="text-gray-500 line-through text-sm mt-1">Rs. {data.discount}</p>
                        {/* Delete icon */}
                        <i className='fa-solid fa-trash-alt mt-3 hover:text-site-color cursor-pointer' onClick={() => deleteHandler(data._id)}></i>
                    </div>
                    <div className='flex items-start me-4'>
                        <div className="flex flex-row gap-1 items-center">
                            <h3 className="text-gray-500 text-sm">Qty:</h3>
                            <h2 className='text-lg font-semibold'>{data.quantity}</h2>
                        </div>
                    </div>
                </div>
            </div>
